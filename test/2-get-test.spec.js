...
describe('Get API Tests using SuperTest', () => {
	...
	it('should fetch the records from Get API using a query param', async () => {
		let response = await request(ecommerce_baseurl).get('/getOrder').query({ product_id: 3 });
		...
		expect(response.body.orders[0].product_name).to.contain('Samsung S24 Ultra');
		expect(response.body.orders[0].product_amount).to.be.greaterThan(4000);
		expect(response.body.orders[0].qty).to.be.greaterThan(0);
		expect(response.body.orders[0].tax_amt).to.be.lessThan(6.00);
	...
	it('should fetch the records from GET API using path parameter', async () => {
		...
		expect(response.body.bookingdates.checkin).to.equal(booking.bookingdates.checkin);
		expect(response.body.bookingdates.checkout).to.equal(booking.bookingdates.checkout);
	...
	it('should successfully authenticate with valid credentials', async () => {
		...
		let response = await request(httpbin_baseurl).get("/basic-auth/user/passwd")
			.set("Accept", "application/json")
			.auth(user, password)
			.expect(200);
	...
	it('should return 401 with invalid credentials', async () => {
		...
		await request(httpbin_baseurl).get("/basic-auth/user/passwd")
			.set("Accept", "application/json")
			.auth(user, password)
			.expect(401);
	});
